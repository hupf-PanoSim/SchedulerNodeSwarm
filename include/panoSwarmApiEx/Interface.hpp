...
/// @brief 控制点类型
enum class KeyPointType : unsigned char {
    Road = 0,
    Free
};
...
/// @brief 返回范围内的RSU, [(id,shape,x,y,z,yaw,pitch,roll,range,fov,coverage,junction)]
using RsuData = std::tuple<int, int, double, double, double, double, double, double, double, double, std::string, std::string>;
...
/// @brief 返回地图坐标投影和偏移
std::tuple<std::string_view, double, double> GetMapProjection();
...
/// @brief 返回车道限速
double GetLaneSpeedLimit(int nLaneIndex);

/// @brief 返回主车行驶轨迹
std::vector<std::tuple<double, double, KeyPointType, int>> GetKeyPoints();

/// @brief 返回目标车位编号，-1表示无效，0表示任意车位
int GetTargetParkingSpot();

/// @brief 返回lane所在的edge
int GetEdgeByLane(int lane);

/// @brief 返回驾驶任务路径点
std::vector<std::pair<double, double>> GetWayPoints();

/// @brief 返回驾驶任务速度表
std::vector<std::pair<double, double>> GetSpeedTable();

/// @brief 返回驾驶任务起点坐标
std::tuple<double, double, double> GetStartPoint();
