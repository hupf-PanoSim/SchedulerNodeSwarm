...
bool ParsePlugin(const T& plugin, const std::string& strDatabaseHome, std::map<std::string, std::int32_t>& mapAgentLink2Cnt)
{
    ...
    return Plugins.LoadPlugin(Plugin::Agent, strLink, mapAgentLink2Cnt[strLink], std::move(mapParameter));
}
...
bool ParseSensor(const T& sensor, const std::string& strDatabaseHome, std::map<std::string, std::int32_t>& mapSensorLink2Cnt)
{
    ...
    return Plugins.LoadPlugin(Plugin::Sensor, strLink, mapSensorLink2Cnt[strLink], std::move(mapParameter));
}

bool CEquipmentXml::Parse(const std::string& strDatabaseHome, const std::string& strExperimentXml)
{
    try {
